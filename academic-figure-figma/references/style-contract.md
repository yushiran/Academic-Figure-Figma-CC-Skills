...
- **A fraction is not 8 pt just because you rendered it at 8 pt.** A text-style fraction sets its digits at **0.7 times** the render size, so `\genfrac` at 8 pt gives 5.6 pt digits, under the floor and visibly blurrier than the plain numerals beside them. Render tick fractions at **11.43 pt** and the digits come out at exactly 8 pt, matching an 8 pt integer, for 14.4 pt of height instead of 11.0. The same arithmetic applies to any sub- or superscript.
- **`\genfrac`'s rule argument is in hundredths of the font size, not in points.** The emitted SVG scales its glyph space by `fontsize/100`, so the printed bar is `rule × fontsize / 100` pt — measured dead linear from rule 0.6 to 8.0. A rule of `0.4` at 8 pt is a **0.03 pt** hairline that offset printing does not reproduce and a screen antialiases into grey; the user caught it by eye and warned it would confuse a reviewer. **Use `rule = 100 × target / fontsize`.**
- **Take the target weight off the paper, not out of the air.** The document already sets these fractions in its prose, so measure one: at 10 pt body a Computer Modern `\tfrac` bar is **0.400 pt** carrying 7 pt digits. Scale that ratio to the figure's 8 pt digits and the target is **0.457 pt**, which at the 11.43 pt render size is `\genfrac{}{}{4.0}{1}{1}{2}` — a 0.458 pt bar with 0.450 pt of clearance. Set the figure's fraction and the paper's side by side at equal digit height and they are the same fraction. *(matplotlib's own `\frac` default measures 0.717 pt at 11.43 pt, which is the right weight for 11.43 pt digits, not for the 8 pt ones the 0.7 reduction actually prints. Do not take it as the reference.)*
- **Never thicken a fraction bar in place.** TeX scales the clearance above and below with the rule, so a bar resized inside the old geometry eats the gap and merges with the numerator's serif: 0.069 → 0.500 pt in place left 0.04 pt of air and read as one blob. Re-render the whole fraction and place it **by its bar**, not by its bounding box — the taller glyph of `7/8` otherwise shifts its box and puts its bar 0.05 pt off the lane its neighbours sit on.
- **Symbols are Computer Modern at 8 pt, placed at 1:1 and never rescaled.** `latex2svg.py` with fontset `cm` writes a viewBox in points and Figma imports one unit as one px, so the symbol on the artboard is exactly its font size (x-height 3.6 pt). **10 pt** for the one hero symbol (the controller's `u_ψ` inside its block). Tick numerals and fractions are maths too: `\genfrac{}{}{4.0}{1}{1}{2}` at 11.43 pt is the paper's `\tfrac12`, not a Unicode ½ in a text font. Never re-set a symbol in sans; never set a mechanism name in maths.
